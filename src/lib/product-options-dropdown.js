...
const DEFAULT_OPTIONS = {
  ...
  qtyContainerClasses: ['d-flex', 'flex-wrap', 'align-items-center', 'mt-2']
};
...
export default class ProductOptionDropdown extends PropObject {
  ...
  build() {
    ...
    // Hide Original Table
    if(!this.options.debug) this.props.productOptionTableDom.classList.add('d-none');
    ...
    qtyContainer.innerHTML = `
      <span class="mr-2">${this.qtyLabel}</span>
    ...
    if(buyBtn) {
      this.props.containerDom.appendChild(buyBtn);
